...
description: (Optional) Description of the texture pack
author: (Optional) Author of the texture pack
copyright: (Optional) Copyright/license info
filename: Filename to write the textures to
image: Name of the texture map image
icon: icon for the texture pack
*/
if (process.argv.length < 3) {
    console.log("Please provide a json file describing a texture!");
    process.exit(1);
}
...
try {
    ...
    if (e.code === "ENOENT") {
        console.log("Could not find texture pack description file!");
        process.exit(1);
    } else {
        throw e;
    }
...
if (typeof(texturePack.name) === "undefined") {
    console.log("This is not a valid texture pack json! The name field is missing!");
    process.exit(1);
}
if (typeof(texturePack.copyright) === "undefined") {
    console.log("This is not a valid texture pack json! The copyright field is missing!");
    process.exit(1);
}
if (typeof(texturePack.filename) === "undefined") {
    console.log("This is not a valid texture pack json! The filename field is missing!");
    process.exit(1);
}
...
async function processFile() {
    let image;
    await Jimp.read(absoluteDir + texturePack.image).then(image2 => image = image2).catch(e => {
        if (e.code === "ENOENT") {
...
function darkenColor(color) {
    return linearToRGB(rgbToLinear(color)*0.25);
}
