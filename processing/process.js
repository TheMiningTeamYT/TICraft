...
/*
Texture pack config file options:
name: (String) Name of the texture pack
version: (Optional) (String) verson of the texture pack
description: (Optional) (String) Description of the texture pack
author: (Optional) (String) Author of the texture pack
copyright: (Optional) (String) Copyright/license info
filename: (String) Filename to write the textures to
image: (String) Name of the texture map image
icon: (Optional) (String) Name of the icon image for the texture pack
*/
if (process.argv.length < 3) {
    console.error("Please provide a json file describing a texture!");
    process.exit(1);
}
...
try {
    ...
    if (e.code === "ENOENT") {
        console.error("Could not find texture pack description file!");
        process.exit(1);
    } else {
        throw e;
    }
...
if (typeof(texturePack.name) === "undefined") {
    console.error("This is not a valid texture pack json! The name field is missing!");
    process.exit(1);
}
if (typeof(texturePack.copyright) === "undefined") {
    console.error("This is not a valid texture pack json! The copyright field is missing!");
    process.exit(1);
}
if (typeof(texturePack.filename) === "undefined") {
    console.error("This is not a valid texture pack json! The filename field is missing!");
    process.exit(1);
}
if (typeof(texturePack.image) === "undefined") {
    console.error("This is not a valid texture pack json! The image field is missing!");
    process.exit(1);
}
if (typeof(texturePack.icon) === "undefined") {
    console.warn("No icon was specified, using the default icon instead.");
...
function darkenColor(color) {
    return linearToRGB(rgbToLinear(color)*0.33);
}
// The gamma value used by TI-OS is 1.8
function rgbToLinear(color) {
    return Math.pow(color, 1.8);
}
...
function linearToRGB(color) {
    ...
    return Math.pow(color, 1/1.8);
}

function rgb888to555(color) {
    color = Math.round(color / 8);
    if (color > 31) {
        color = 31;
    }
    return color;
...
function rgb888to555full(color) {
    return (rgb888to555(color[0]) << 10) + (rgb888to555(color[1]) << 5) + rgb888to555(color[2]);
}

function rgb888to565full(color) {
    return (rgb888to555(color[0]) << 11) + (rgb888to565(color[1]) << 5) + rgb888to555(color[2]);
}

function darkenRgb888to555full(color) {
    return (rgb888to555(darkenColor(color[0])) << 10) + (rgb888to555(darkenColor(color[1])) << 5) + rgb888to555(darkenColor(color[2]));
}
/*
** The crc32 is licensed under the Apache License, Version 2.0, and a copy of the license is included in this file.
**
** Author:Wang Yaofu voipman@qq.com
** Ported to JavaScript by Logan Cole loganisamazing@outlook.com
...
// s: buffer
function crc32(s, len) {
    let crc32val = 0xFFFFFFFF;
    for (i = 0;  i < len;  i++) {
...
async function processFile() {
    let image;
    let imagePromise = Jimp.read(absoluteDir + texturePack.image).then(image2 => {
        image = image2
    }).catch(e => {
        if (e.code === "ENOENT") {
            console.error("Could not find texture map!");
            process.exit(1);
        } else {
            throw e;
        }
    ...
    let iconImagePromise = Jimp.read(icon).then(image => {
        ...
        if (e.code === "ENOENT") {
            console.warn("Could not find the specified icon, using the default icon instead...");
            iconImagePromise = Jimp.read("unknown_pack.png").then(image => {
                iconImage = image
            }).catch(e => {
                if (e.code === "ENOENT") {
                    console.error("Could not find default icon!");
                    process.exit(1);
                } else {
                    throw e;
                }
    ...
    if (typeof(texturePack.tolerence) === "number") {
        tolerence = texturePack.tolerence;
    }
    await imagePromise;
    ...
    while (indexedImageBuffer.length < 270) {
        indexedImageBuffer.writeUInt16LE(0);
    }
    for (var i = 0; i < palette.length; i++) {
        let color = darkenRgb888to555full(palette[i]);
        indexedImageBuffer.writeUInt16LE(color);
    }
    ...
    indexedImageBuffer.writeStringNT(texturePack.name);
    if (typeof(texturePack.description) === "string") {
        indexedImageBuffer.writeStringNT(texturePack.description);
    } else {
        indexedImageBuffer.writeStringNT("");
    }
    if (typeof(texturePack.author) === "string") {
        indexedImageBuffer.writeStringNT(texturePack.author);
    } else {
        indexedImageBuffer.writeStringNT("");
    }
    if (typeof(texturePack.version) === "string") {
        indexedImageBuffer.writeStringNT(texturePack.version);
    } else {
        indexedImageBuffer.writeStringNT("");
    }
    indexedImageBuffer.writeStringNT(texturePack.copyright);
    indexedImageBuffer.writeInt32LE(crc32(indexedImageBuffer.toBuffer(), indexedImageBuffer.length));
    fs.writeFileSync(absoluteDir + texturePack.filename + ".bin", indexedImageBuffer.toBuffer());
    // Potentially unsafe
    exec("convbin -j bin -k 8xv -r -i \"" + absoluteDir + texturePack.filename + ".bin\" -o \"" + absoluteDir + texturePack.filename + ".8xv\" -n " + texturePack.filename.slice(0, 8));
    console.log("Final texture pack was written to " + absoluteDir + texturePack.filename + ".8xv");
    image.write(absoluteDir + "TextureMap--Converted.png");
}