...
uint8_t* coal_ore;
uint8_t* cobblestone;
uint8_t* cobblestone_mossy;
uint8_t* crafting_table_top;
uint8_t* crafting_table_front;
uint8_t* crafting_table_side;
uint8_t* dirt;
uint8_t* furnace_top;
uint8_t* furnace_front;
uint8_t* furnace_side;
uint8_t* gold_block;
uint8_t* grass_top;
uint8_t* grass_side;
uint8_t* gravel;
...
uint8_t* iron_ore;
uint8_t* jukebox_top;
uint8_t* jukebox_side;
uint8_t* leaves_oak;
uint8_t* log_oak_top;
uint8_t* log_oak;
uint8_t* planks_oak;
uint8_t* sand;
uint8_t* smooth_stone_top;
uint8_t* smooth_stone_side;
uint8_t* sponge;
uint8_t* stone;
uint8_t* tnt_top;
uint8_t* tnt_side;
uint8_t* tnt_bottom;
uint8_t* glass;
uint8_t* diamond_block;
uint8_t* lapis_block;
uint8_t* emerald_block;
uint8_t* stonebrick;
uint8_t* snow;
uint8_t* pumpkin_top;
uint8_t* pumpkin_side;
uint8_t* red_wool;
uint8_t* orange_wool;
uint8_t* yellow_wool;
uint8_t* lime_green_wool;
uint8_t* green_wool;
uint8_t* cyan_wool;
uint8_t* light_blue_wool;
uint8_t* blue_wool;
uint8_t* pink_wool;
uint8_t* magenta_wool;
uint8_t* purple_wool;
uint8_t* black_wool;
uint8_t* brown_wool;
uint8_t* grey_wool;
uint8_t* light_grey_wool;
uint8_t* white_wool;
uint8_t* quartz_top;
uint8_t* hay_block_top;
uint8_t* hay_block_side;
uint8_t* bacon;
uint8_t* pumpkin_face;
...
void initPalette() {
    uint8_t texturePackHandle = ti_Open("TEXTURES", "r");
    if (!texturePackHandle) {
        gfx_End();
        os_ClrHomeFull();
        os_PutStrFull("Texture pack is missing -- cannot continue. Please load a texture pack and try again. Press any key to exit.");
        while (!os_GetCSC());
        exit(1);
    }
    ti_SetArchiveStatus(true, texturePackHandle);
    uint16_t* texPalette = (uint16_t*) ti_GetDataPtr(texturePackHandle);
    textureMap = (uint8_t*) (texPalette + 256);
    ti_Close(texturePackHandle);
    memcpy(gfx_palette, texPalette, 512);
    bedrock = &textureMap[0];
    bookshelf = &textureMap[1*256];
    bricks = &textureMap[2*256];
    coal_ore = &textureMap[3*256];
    cobblestone = &textureMap[4*256];
    cobblestone_mossy = &textureMap[5*256];
    crafting_table_top = &textureMap[6*256];
    crafting_table_front = &textureMap[7*256];
    crafting_table_side = &textureMap[8*256];
    dirt = &textureMap[9*256];
    furnace_top = &textureMap[10*256];
    furnace_front = &textureMap[11*256];
    furnace_side = &textureMap[12*256];
    gold_block = &textureMap[13*256];
    grass_top = &textureMap[14*256];
    grass_side = &textureMap[15*256];
    gravel = &textureMap[16*256];
    iron_block = &textureMap[17*256];
    iron_ore = &textureMap[18*256];
    jukebox_top = &textureMap[19*256];
    jukebox_side = &textureMap[20*256];
    leaves_oak = &textureMap[21*256];
    log_oak_top = &textureMap[22*256];
    log_oak = &textureMap[23*256];
    planks_oak = &textureMap[24*256];
    sand = &textureMap[25*256];
    smooth_stone_top = &textureMap[26*256];
    smooth_stone_side = &textureMap[27*256];
    sponge = &textureMap[28*256];
    stone = &textureMap[29*256];
    tnt_top = &textureMap[30*256];
    tnt_side = &textureMap[31*256];
    tnt_bottom = &textureMap[32*256];
    glass = &textureMap[33*256];
    diamond_block = &textureMap[34*256];
    lapis_block = &textureMap[35*256];
    emerald_block = &textureMap[36*256];
    stonebrick = &textureMap[37*256];
    snow = &textureMap[38*256];
    pumpkin_top = &textureMap[39*256];
    pumpkin_side = &textureMap[40*256];
    pumpkin_face = &textureMap[41*256];
    red_wool = &textureMap[42*256];
    orange_wool = &textureMap[43*256];
    yellow_wool = &textureMap[44*256];
    lime_green_wool = &textureMap[45*256];
    green_wool = &textureMap[46*256];
    cyan_wool = &textureMap[47*256];
    light_blue_wool = &textureMap[48*256];
    blue_wool = &textureMap[49*256];
    pink_wool = &textureMap[50*256];
    magenta_wool = &textureMap[51*256];
    purple_wool = &textureMap[52*256];
    black_wool = &textureMap[53*256];
    brown_wool = &textureMap[54*256];
    grey_wool = &textureMap[55*256];
    light_grey_wool = &textureMap[56*256];
    white_wool = &textureMap[57*256];
    quartz_top = &textureMap[58*256];
    hay_block_top = &textureMap[59*256];
    hay_block_side = &textureMap[60*256];
    bacon = &textureMap[61*256];
    // ugh
